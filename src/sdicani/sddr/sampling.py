...
def mcmc(
    ...
    with p() as pool:
        ...
        chain = _burn_and_thin_chain(chain, config.burn_in, config.thin)
        samples = np.ascontiguousarray(chain.reshape(-1, ndim))

        lnprob_chain = sampler.get_log_prob()  # shape (nsteps, nwalkers)
        lnprob_chain = _burn_and_thin_chain(lnprob_chain, config.burn_in, config.thin)
        lnprob = np.ascontiguousarray(lnprob_chain.reshape(-1))

    return samples, lnprob


def _burn_and_thin_chain(chain: np.ndarray, burn_in: int, thin: int) -> np.ndarray:
